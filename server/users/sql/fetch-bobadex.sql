...
INNER JOIN LATERAL (
    ...
    INNER JOIN bobadex_season_secret_identities
      ON bobadex_seasons.id = bobadex_season_secret_identities.bobadex_season_id
          AND si.id = bobadex_season_secret_identities.secret_identity_id
    LEFT JOIN user_thread_identities uti 
      ON uti.identity_id = si.id 
          AND uti.user_id = (SELECT id FROM users WHERE firebase_id = $/firebase_id/)
    GROUP BY si.id
) ordered_identities ON TRUE
